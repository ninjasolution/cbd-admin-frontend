...
function PaymentOption() {
  ...
  return (
    <div>
      ...
      <div>
        ...
        </p> */}
        {
          categories.map((col, idx) =>
            <p key={idx} onClick={() => setcollection(col)} className={`flex items-center space-x-1 cursor-pointer px-3 mb-1 py-3 border-inherit border text-base ${col.collection === collection.collection && "main-bg"}`}>
      ...
      <div className="flex items-center justify-center space-x-3 py-10 border border-inherit border-l-0 border-r-0 mt-3">
        <div className="main-bg w-20 text-center flex justify-center items-center rounded-full py-4 cursor-pointer hover:bg-[#4b4b4b]" onClick={() => setCount(count === 0 ? 0 : count - 1)}>
          <Minuse className="text-white" width={30} height={30} />
        </div>

        <div className="bg-[#4b4b4b] text-white  text-5xl py-2 px-10">{count}</div>
        <div className="main-bg w-20 text-center flex justify-center items-center rounded-full py-4 cursor-pointer hover:bg-[#4b4b4b]" onClick={() => setCount(count + 1)}>
          <PlushIcon className="text-white" width={30} height={30} />
        </div>
      </div>

      <div className="text-right pt-7 md:pt-10">
        ...
        <div className="mt-5 text-right">
          <CheckBox label={
            <p className='text-right'>
              I agree with the Cannerald{" "} <Link to="/" className="color-2">{" "} Terms and Conditions </Link>
            </p>
          } right />
